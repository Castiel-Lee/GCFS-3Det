...
def sample_adaptive_balance_loss(pred, gt, mask=None, theta=0.1):
    """
    Args:
        pred: (batch x c x h x w)
        gt: (batch x c x h x w)
    ...
    pos_inds = gt.eq(1).float()
    neg_inds = gt.lt(1).float()
    hn_inds = pred.gt(theta).float()

    if mask is not None:
        mask = mask[:, None, :, :].float()
        num_pos = (pos_inds.float() * mask).sum()
        num_neg = (neg_inds.float() * mask).sum()
        num_hn = (hn_inds.float() * mask).sum()
    else:
        num_pos = pos_inds.float().sum()
        num_neg = neg_inds.float().sum()
        num_hn = hn_inds.float().sum()

    # neg_weights = torch.pow(1 - gt, 4)
    pos_weights = torch.sqrt(1-pred)
    neg_weights = num_pos / (num_pos + num_neg)
    hn_weights = num_hn / (num_pos + num_hn) if num_hn.item() != 0 else 0

    loss = 0
    # print('hn_weights:', hn_weights, 'num_hn:', num_hn.item(), 'num_pos:', num_pos.item())
    pos_loss = pos_weights * gt * torch.log(pred)  * pos_inds
    neg_loss = neg_weights * (1 - gt) * torch.log(1 - pred) * neg_inds * (1 - hn_inds)
    hn_loss = hn_weights * (1 - gt) * torch.log(1 - pred) * neg_inds * hn_inds 

    if mask is not None:
        mask = mask[:, None, :, :].float()
        pos_loss = pos_loss * mask
        neg_loss = neg_loss * mask
        hn_loss = hn_loss * mask 
    ...
    if num_pos == 0:
        loss = loss - (neg_loss + hn_loss)
    else:
        loss = loss - (pos_loss + neg_loss + hn_loss) / num_pos
    return loss

class SampleBlanceLossCenterNet(nn.Module):
    def __init__(self):
        super(SampleBlanceLossCenterNet, self).__init__()
        self.sample_adaptive_balance_loss = sample_adaptive_balance_loss

    def forward(self, out, target, mask=None):
        return self.sample_adaptive_balance_loss(out, target, mask=mask)



def _reg_loss(regr, gt_regr, mask, heatmap_weight=None):
    """
    Refer to https://github.com/tianweiy/CenterPoint
    L1 regression loss
    Args:
    ...
    if heatmap_weight is not None:
        heatmap_weight = heatmap_weight * mask
        heatmap_weight = heatmap_weight.expand(heatmap_weight.size(0), heatmap_weight.size(1), regr.size(2))
...
class RegLossCenterNet(nn.Module):
    ...
    def forward(self, output, mask, ind=None, target=None, heatmap_weight=None):
        """
        Args:
            ...
            heatmap_weight: : (batch x dim x h x w)
        Returns:
        """
        if ind is None:
            pred = output
            assert heatmap_weight is None
        else:
            pred = _transpose_and_gather_feat(output, ind)
            heatmap_weight = _transpose_and_gather_feat(heatmap_weight, ind) if heatmap_weight is not None else None

        loss = _reg_loss(pred, target, mask, heatmap_weight)
        return loss
