...
class PVRCNNHead_CLS_PROTOTYPE(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, class_names, num_class=1, **kwargs):
    ...
    def forward(self, batch_dict):
        ...
        if batch_dict.get('add_roi_boxes', False):
            raise NotImplementedError('ADDING BOXES PVRCNN, TO DO!')
            print('add_roi_boxes')
        ...
        if self.model_cfg.get('CLS_PROTOTYPES_ATTEN', None):
            raise NotImplementedError('CLS_PROTOTYPES_ATTEN, TO DO!')
        
        if self.model_cfg.get('LEARNABLE_CLS_PROTOTYPES_ATTEN', None):
            # print('LEARNABLE_CLS_PROTOTYPES_ATTEN')
            proto_atten = self.cls_embedding.weight#.clone()
            shared_features = self.cls_embedding_reweight_layer(
                shared_features.squeeze(2).unsqueeze(0), 
                proto_atten.unsqueeze(0)).squeeze(0).unsqueeze(-1)
