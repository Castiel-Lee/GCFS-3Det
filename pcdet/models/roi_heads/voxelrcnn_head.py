...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        """
        ### init
        # with open('/home/shuangzhi/temp2.pkl', 'wb') as f:
        #     pickle.dump([], f)
        
        # if not self.training:
        #     with open('/home/shuangzhi/rois_orgmth.pkl', 'rb') as f:
        #         infos = pickle.load(f)
        #     infos.append({
        #         'rois': batch_dict['rois'].cpu(),
        #         'roi_labels': batch_dict['roi_labels'].cpu(),
        #         'roi_scores': batch_dict['roi_scores'].cpu(),
        #         'frame_id': batch_dict['frame_id'][0]
        #     })
        #     with open('/home/shuangzhi/rois_orgmth.pkl', 'wb') as f:
        #         pickle.dump(infos, f)
        ...
        # if not self.training:
        #     with open('/home/shuangzhi/rois_withadd_orgmth.pkl', 'rb') as f:
        #         infos = pickle.load(f)
        #     infos.append({
        #         'rois': batch_dict['rois'].cpu(),
        #         'roi_labels': batch_dict['roi_labels'].cpu(),
        ...
        #         'frame_id': batch_dict['frame_id'][0]
        #     })
        #     with open('/home/shuangzhi/rois_withadd_orgmth.pkl', 'wb') as f:
        #         pickle.dump(infos, f)
        ...
        # if self.model_cfg.get('ADD_NOISE_ON_PROPOSAL_XY_TRAINING', False) and self.training:
        #     add_noise_cfg = self.model_cfg.ADD_NOISE_ON_PROPOSAL_XY_TRAINING
        #     batch_size = batch_dict['rois'].shape[0]
        #     num_obj = batch_dict['rois'].shape[1]
        #     for i in range(batch_size):
        #         for j in range(num_obj):
        #             if batch_dict['roi_labels'][i,j] in add_noise_cfg.atten_cls_idx:
        #                 # loc x y
        #                 batch_dict['rois'][i,j,0:2] += ((torch.rand(2)*2 - 1)*add_noise_cfg.noise).to(dtype=batch_dict['rois'].dtype).cuda()
        #                 # siz x y
        #                 batch_dict['rois'][i,j,3:5] *= ((torch.rand(2)*2 - 1)*add_noise_cfg.noise + 1).to(dtype=batch_dict['rois'].dtype).cuda()
        #                 # print(batch_dict['roi_labels'][i,j])
        
        # if self.model_cfg.get('ADD_NOISE_ON_PROPOSAL_XYH_TRAINING', False) and self.training:
        #     add_noise_cfg = self.model_cfg.ADD_NOISE_ON_PROPOSAL_XYH_TRAINING
        #     batch_size = batch_dict['rois'].shape[0]
        #     num_obj = batch_dict['rois'].shape[1]
        #     for i in range(batch_size):
        #         for j in range(num_obj):
        #             if batch_dict['roi_labels'][i,j] in add_noise_cfg.atten_cls_idx:
        #                 # heading
        #                 heading_noise = (torch.rand(1)*2 - 1).to(dtype=batch_dict['rois'].dtype).cuda() * add_noise_cfg.H_noise
        #                 batch_dict['rois'][i,j,6] += heading_noise.item()
        #                 batch_dict['rois'][i,j,6] = (batch_dict['rois'][i,j,6] + np.pi) % (2 * np.pi) - np.pi
        #                 targets_dict['gt_of_rois'][i,j,6] -= heading_noise.item()
        #                 targets_dict['gt_of_rois'][i,j,6] = (targets_dict['gt_of_rois'][i,j,6] + np.pi * 0.5) % np.pi - np.pi * 0.5
        #                 targets_dict['gt_of_rois'][i,j,0:3] = common_utils.rotate_points_along_z(
        #                     points=targets_dict['gt_of_rois'][i,j,0:3].view(1, 1, 3), angle=-heading_noise.view(-1)
        #                 ).view(3)
        #                 # loc x y
        #                 xyz_shift_noise = (torch.rand(3)*2 - 1).to(dtype=batch_dict['rois'].dtype).cuda() * add_noise_cfg.XY_noise_ratio * batch_dict['rois'][i,j, 3:6]
        #                 xyz_shift_noise[2] = 0 # only shift XY
        #                 global_shift = common_utils.rotate_points_along_z(
        #                     points=xyz_shift_noise.view(1, 1, xyz_shift_noise.shape[-1]), angle=batch_dict['rois'][i,j,6].view(-1)
        #                 ).view(xyz_shift_noise.shape[-1])
        #                 batch_dict['rois'][i,j,0:3] += global_shift
        #                 targets_dict['gt_of_rois'][i,j,0:3] -= xyz_shift_noise
        #                 # size x y
        #                 xy_size_scalor = ((torch.rand(2)*2 - 1)*add_noise_cfg.DIM_noise_ratio + 1).to(dtype=batch_dict['rois'].dtype).cuda()
        #                 batch_dict['rois'][i,j,3:5] *= xy_size_scalor
                        

        # RoI aware pooling
        pooled_features = self.roi_grid_pool(batch_dict)  # (BxN, 6x6x6, C)

        # # lsz: output object-level gt-roi for analysis
        # if not self.training: 
        #     import os
        #     import pickle

        #     frame_names = ''
        #     for item in batch_dict['frame_id']:
        #         frame_names += item.split('.')[0] + '&'
            
        #     ROOT_SAVE_DIR = '/dataset/shuangzhi/feature_output'

        #     for source_index in [0,2]:
        #         ## DIR
        #         save_dir = (ROOT_SAVE_DIR + '/object_features/source_%d/DIR/%s' % (source_index, batch_dict['dataset_domain']))
        #         if not os.path.exists(save_dir):
        #             os.makedirs(save_dir)
        #         with open((save_dir + '/%s.pkl' % frame_names), 'wb') as f:
        #             dump_data = self.gt_grid_pool_output(batch_dict, output_index=source_index,  dsr_used=False)         
        #             pickle.dump(dump_data, f)

        #         # ## DSR
        #         # save_dir = (ROOT_SAVE_DIR + '/object_features/source_%d/DSR/%s' % (source_index, batch_dict['dataset_domain']))
        #         # if not os.path.exists(save_dir):
        #         #     os.makedirs(save_dir)
        #         # with open((save_dir + '/%s.pkl' % frame_names), 'wb') as f:
        #         #     dump_data = self.gt_grid_pool_output(batch_dict, output_index=source_index,  dsr_used=True)         
        #         #     pickle.dump(dump_data, f)

        #     # print(batch_dict['frame_id'])
        #     # if batch_dict['frame_id'][0][-7]=='4':
        ...
        if not self.training:
            ...
            # with open('/home/shuangzhi/rois_after2stage.pkl', 'rb') as f:
            #     infos = pickle.load(f)
            # infos.append({
            #     'rois': batch_box_preds.cpu(),
            #     'roi_labels': batch_dict['roi_labels'].cpu(),
            #     'roi_scores': batch_cls_preds.cpu(),
            #     'frame_id': batch_dict['frame_id'][0]
            # })
            # with open('/home/shuangzhi/rois_after2stage.pkl', 'wb') as f:
            #     pickle.dump(infos, f)

            # for i in range(batch_cls_preds.shape[0]):
            #     for j in range(batch_cls_preds.shape[1]):
            #         if batch_dict['roi_labels'][i,j] in [11,12,13,14]:
            #             batch_cls_preds[i,j,0] += 0.2
