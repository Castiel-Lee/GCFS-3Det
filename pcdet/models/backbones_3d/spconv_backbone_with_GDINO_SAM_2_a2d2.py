...
from .GDINO_utils import score_down_logit, rename_str_in_phrases

from pcdet.utils.box2d_utils import pairwise_iou

import numpy as np

from .gen_3D_box_a2d2 import generate_box_from_points
# from .gen_3D_box_a2d2_faster import generate_box_from_points
...
class VoxelBackBone8x_ImgGDinoSam_2_a2d2(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, **kwargs):
        ...
        for layer_id in self.model_cfg.MULTI_SRC_CHANNEL:
            ...
            single_BN_RELU = nn.Sequential(
                norm_fn(self.backbone_channels[layer_id]),
                nn.ReLU(),
            )
    ...
    def gen_img_seg_feat_2D(self, image_cur, boxes=None, labels=None, score_rois=None):
        ...
        if use_gdino_box:
            ...
            boxes_dino_first, logits_dino_first, phrases_dino_first = gdino_processing(
                ...
                denial_labels=first_config['repel_labels'],
            )
            ...
            boxes_dino_second, logits_dino_second, phrases_dino_second = gdino_processing(
                ...
                denial_labels=second_config['repel_labels'],
            )
        ...
        if boxes.shape[0] > 0:
            ...
            if use_gdino_box:
                boxes, masks, labels, score_rois, score_masks =\
                    removed_overlap_low_priority_obj(boxes, masks, labels, score_rois, score_masks, 
                    txt_to_cls_dict=self.model_cfg.TEXT_TO_CLS)
    ...
    def construct_multimodal_features(self, x, x_rgb, batch_dict, voxel_stride_XYZ, fuse_sum=False):
        ...
        for b in range(batch_size):
            ...
            if fuse_sum:
                ...
                repteat_c = int(voxel_features_sparse.shape[-1] / (image_features_batch.shape[-1] + distance_3d_batch.shape[-1]))
                image_with_voxelfeature = voxel_features_sparse + torch.cat((image_features_batch, distance_3d_batch),dim=-1).repeat(1,repteat_c)
            else:
    ...
    def image_pc_feat_fusion(self, batch_dict, use_2d_gt):
        ...
        fused_feat_batch = self.construct_multimodal_features(
            ...
            voxel_stride_XYZ = torch.tensor([8, 8, 16], dtype=int).cuda(), 
            fuse_sum=True,
        )
        ...
        for k, layer_id in enumerate(self.model_cfg.MULTI_SRC_CHANNEL):
            ...
            fused_feat_batch = self.construct_multimodal_features(
                ...
                x_rgb = x_img, 
                batch_dict=batch_dict, 
                voxel_stride_XYZ = torch.tensor([batch_dict['multi_scale_3d_strides'][layer_id]]*3, dtype=int).cuda(), 
                fuse_sum=True,
            )
