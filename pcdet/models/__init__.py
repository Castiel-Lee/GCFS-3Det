...
def build_network(model_cfg, num_class, dataset, class_names):
...
def load_data_to_gpu(batch_dict):
    for key, val in batch_dict.items():
        if key == 'camera_imgs':
            ...
            continue
        elif key in ['frame_id', 'metadata', 'calib', 'image_paths', 'image_paths_fc','ori_shape','img_process_infos']:
            continue
        elif key in ['images']:
            batch_dict[key] = kornia.image_to_tensor(val).float().cuda().contiguous()
        elif key in ['image_shape']:
            batch_dict[key] = torch.from_numpy(val).int().cuda()
        elif key in ['db_flag', 'dataset_domain', 'domain_aug', 'common_classes', 'novel_classes']:
            continue
        else:
