...
class KittiDataset(DatasetTemplate):
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        class_names = [classes_map[item] for item in class_names]
        assert class_names.__len__() == list(set(class_names)).__len__()
        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.kitti_infos]
  
        for anno in eval_gt_annos:
            anno['name'] = np.vectorize(lambda x: classes_map.get(x, x))(anno['name'])
        
        for anno in eval_det_annos:
            if anno['name'].__len__() > 0:
                anno['name'] = np.vectorize(lambda x: classes_map.get(x, x))(anno['name'])
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            annos = info['annos']
            if self.dataset_cfg.get('DIR_USE_LIDAR_BOX', False):
                gt_names = annos['name']
