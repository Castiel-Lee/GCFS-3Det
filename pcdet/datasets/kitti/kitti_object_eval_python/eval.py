...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck', 'tram',
        'construction_vehicle', 'bus', 'trailer', 'barrier', 'motorcycle', 'bicycle', 'traffic_cone', 
        'utilityvehicle', 'sign', 'large_vehicle', 'construction_barrel',
    ]
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
...
def eval_class(gt_annos,
    ...
    # i_sample=407
    # print(overlaps[i_sample].shape, dt_annos[i_sample]['name'], gt_annos[i_sample]['name'])
    # flag_dt = dt_annos[i_sample]['name']=='Person_sitting'
    # flag_gt = gt_annos[i_sample]['name']=='Person_sitting'
    # temp=overlaps[i_sample][flag_dt,:]
    # temp=temp[:,flag_gt]
    # print(temp)
...
def do_eval(gt_annos,
    ...
    # print('precision', ret['precision'][11].mean(axis=0))
    # print('recall', ret['recall'][11].mean(axis=0))
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    overlap = np.array([[0.7, 0.5, 0.5, 0.5, 0.3, 0.5, 0.5], 
                        [0.7, 0.5, 0.5, 0.5, 0.3, 0.5, 0.5],
                        [0.7, 0.5, 0.5, 0.5, 0.3, 0.5, 0.5]])

    overlap_2 = np.array([[0.5, 0.3, 0.5, 0.5, 0.5, 0.3, 0.5, 0.3, 0.3, 0.5, 0.5], 
                        [0.5, 0.3, 0.5, 0.5, 0.5, 0.3, 0.5, 0.3, 0.3, 0.5, 0.5],
                        [0.5, 0.3, 0.5, 0.5, 0.5, 0.3, 0.5, 0.3, 0.3, 0.5, 0.5]])

    overlap = np.concatenate((overlap, overlap_2), axis=1)
    # overlap = np.zeros((3, 18)) + 0.5

    min_overlaps =  overlap[np.newaxis, ...] # [1, 3, N_cls]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
        2: 'Cyclist',
