    ...
    def __init__(self, root_path, sampler_cfg, class_names, logger=None, client=None, oss_flag=False):
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            if not self.oss_flag:
                ...
                # for argoverse 2
                if self.sampler_cfg.get('PRE_TEXT', False):
                    db_info_path = os.path.join(self.sampler_cfg.PRE_TEXT, db_info_path)
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                bin_file_path = info['path']
                # for argoverse 2
                if self.sampler_cfg.get('PRE_TEXT', False):
                    bin_file_path = os.path.join(self.sampler_cfg.PRE_TEXT, bin_file_path)
                    
                file_path = os.path.join(self.root_path, bin_file_path)
                if self.oss_flag:
        ...
        len_all,  len_add = gt_boxes.shape[0], sampled_gt_boxes.shape[0]
        data_dict['gt_boxes_added'] = np.array([False] * (len_all - len_add) + [True] * len_add, dtype=bool)
        # id_pt = 0
