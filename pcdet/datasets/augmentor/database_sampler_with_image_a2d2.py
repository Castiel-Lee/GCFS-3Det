...
from pcdet.datasets.a2d2.a2d2_utils import Calibration as a2d2_calib
...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        ## calib for all
        self.calib_constant = a2d2_calib(str(self.root_path / 'cams_lidars.json'))
        # print('*********'*10)
    ...
    def undistort_gt_box_2d(self, db_infos, image_size):      
        new_db_infos = {}
        for cls_name, dinfos in db_infos.items():
            new_dinfos = []
            for i in range(len(db_infos[cls_name])):
                new_info = copy.deepcopy(db_infos[cls_name][i])
                new_info['bbox'] = self.calib_constant.undistort_boxes(new_info['bbox'][np.newaxis, ...])[0]
                new_info['bbox'] = self.calib_constant.img_boxes_constraint(
                    new_info['bbox'][np.newaxis, ...], (int(image_size[0]), int(image_size[1])))[0]
                # remove no area box
                area = (new_info['bbox'][2] - new_info['bbox'][0]) * (new_info['bbox'][3] - new_info['bbox'][1])
                if area > 0:
                    new_dinfos.append(new_info)
            if len(new_dinfos) > 0:
                new_db_infos[cls_name] = new_dinfos  
                if self.logger is not None:
                    self.logger.info('Database filter by undistortion %s: %d => %d' % (cls_name, len(db_infos[cls_name]), len(new_db_infos[cls_name])))
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, mv_height, sampled_gt_boxes2d, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.sampler_cfg.get('POINT_DROP', False):
                for drop_item in self.sampler_cfg.POINT_DROP:
                    obj_cls, drop_range = drop_item.split(':')
                    drop_range = float(drop_range)
                    if info['name'] == obj_cls:
                        # print(obj_cls, obj_points.shape)
                        drop_rate = np.random.uniform(0, drop_range)
                        N_pts = obj_points.shape[0]
                        num_pts_keep = int(N_pts * (1 - drop_rate))
                        indices_keep = np.random.choice(N_pts, num_pts_keep, replace=False)
                        obj_points = obj_points[indices_keep]
                        # print(obj_points.shape)

            if self.aug_with_img:
                # calib_file = kitti_common.get_calib_path(int(info['image_idx']), self.root_path, relative_path=False)
                # sampled_calib = calibration_kitti.Calibration(calib_file, oss_flag=False)
                sampled_calib = self.calib_constant
            ...
            if self.point_refine:
                ...
                points_lidar = data_dict['calib'].img_to_lidar(points_2d, depth_2d)
                obj_points[:, :3] = points_lidar
                # # align calibration metrics for boxes
                # box3d_raw = sampled_gt_boxes[idx].reshape(1,-1)
                # box3d_coords = box_utils.boxes_to_corners_3d(box3d_raw)[0] # box_3d corners
                # box3d_box, box3d_depth = sampled_calib.lidar_to_img(box3d_coords) # 2d corners and depth
                # box3d_coord_rect = data_dict['calib'].img_to_rect(box3d_box[:,0], box3d_box[:,1], box3d_depth) # 3d corners rect
                # box3d_rect = box_utils.corners_rect_to_camera(box3d_coord_rect).reshape(1,-1) # box rect
            ...
            if self.aug_with_img:
                ...
                raw_image, _ = self.calib_constant.undistort_image_and_boxes(raw_image, img_normed=False) # undistort raw_img
                ...
                new_box = sampled_gt_boxes2d[idx].astype(np.int)
                new_box[0::2] = np.clip(new_box[0::2], a_min=0, a_max=raw_image.shape[1])
                new_box[1::2] = np.clip(new_box[1::2], a_min=0, a_max=raw_image.shape[0])
                img_crop2d = raw_image[new_box[1]:new_box[3],new_box[0]:new_box[2]] / 255 # normlize
    ...
    def __call__(self, data_dict):
        ...
        """
        # restore original 3D annotation and points
        if self.sampler_cfg.get('SHIFT_COOR', False):
            data_dict['gt_boxes'][:, 0:3] -= self.sampler_cfg.SHIFT_COOR
            data_dict['points'][:, 0:3] -= np.array(self.sampler_cfg.SHIFT_COOR, dtype=np.float32)
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                if self.aug_with_img:
                    ...
                    # sampled_boxes2d = data_dict['calib'].img_boxes_constraint(sampled_boxes2d, data_dict['images'].shape[:2])
                    ...
                    sampled_boxes2d = data_dict['calib'].boxes3d_lidar_to_boxes2d_img(sampled_boxes, data_dict['images'].shape[:2])
        ...
        if total_valid_sampled_dict.__len__() > 0:
            data_dict = self.add_sampled_boxes_to_scene(data_dict, 
                                                        sampled_gt_boxes,  # touch the plane if USE_ROAD_PLANE
                                                        sampled_mv_height, # if point_shift, it exist in height data
                                                        sampled_gt_boxes2d,
                                                        total_valid_sampled_dict)

        data_dict.pop('gt_boxes_mask')

        # cross-datasets 3D annotation and points
        if self.sampler_cfg.get('SHIFT_COOR', False):
            data_dict['gt_boxes'][:, 0:3] += self.sampler_cfg.SHIFT_COOR
            data_dict['points'][:, 0:3] += np.array(self.sampler_cfg.SHIFT_COOR, dtype=np.float32)
        
        # id_pt = 0
        # np.save('/home/shuangzhi/points%d' %id_pt , data_dict['points'])
        # np.save('/home/shuangzhi/gt_boxes%d' %id_pt , data_dict['gt_boxes'])
