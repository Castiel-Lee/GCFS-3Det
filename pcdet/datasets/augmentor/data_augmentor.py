...
from . import augmentor_utils, database_sampler, database_sampler_adv, database_sampler_with_image, database_sampler_with_image_a2d2
import random
import time
...
class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None, oss_flag=False):
        ...
        self.multi_augmentation_list = None
    ...
    def gt_sampling_adv(self, config=None):
        db_sampler = database_sampler_adv.DataBaseSampler(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger,
    ...
    def gt_sampling_with_image(self, config=None):
        db_sampler = database_sampler_with_image.DataBaseSampler(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger,
    ...
    def gt_sampling_with_image_a2d2(self, config=None):
        db_sampler = database_sampler_with_image_a2d2.DataBaseSampler(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger,
    ...
    def random_object_scaling(self, data_dict=None, config=None):
        ...
        for obj_class in list(config['SCALE_UNIFORM_NOISE'].keys()):
            ...
            points, gt_boxes[mask_cls_obj] = augmentor_utils.scale_pre_object(
                gt_boxes[mask_cls_obj], points,
    ...
    def random_object_scaling_multi_class(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_object_scaling_multi_class, config=config)
    ...
    def normalize_object_size_3objs_without_intensity(self, data_dict=None, config=None):
        ...
        # temp_boxes = copy.deepcopy(data_dict['gt_boxes'])
        ...
        if data_dict['gt_boxes'].shape[1]>7 and data_dict['gt_boxes'].shape[0]>0:
        ...
        # print('processed difference',data_dict['gt_boxes']-temp_boxes)
    ...
    def forward(self, data_dict):
        ...
        # if data_dict['frame_id'] == '001262':
        #     raise NotImplementedError('stop')

        # print(data_dict['gt_boxes'].shape)
        # print(data_dict['gt_names'])
        # print('common_classes:', data_dict['common_classes'], '\tnovel_classes:', data_dict['novel_classes'])
        # gt_info_stat = {}
    ...
    def vf_adversarial_pert(self, data_dict=None, config=None):
        ...
        # id_pt = np.random.choice(5)
        # np.save('/home/shuangzhi/points%d' %id_pt , data_dict['points'])
        # np.save('/home/shuangzhi/gt_boxes%d' %id_pt , boxes_selected)

        return data_dict
    
    def sample_Nways_random(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.sample_Nways_random, config=config)
        
        # points = data_dict['points']
        names = data_dict['gt_names']
        del_flag = np.zeros(names.shape[0], dtype=np.bool_)
        sampled_cls = random.sample(config.NOVEL_CLS, config.N_way)
        saved_cls = config.COMMON_CLS + sampled_cls
        for i in range(names.shape[0]):
            if names[i] not in saved_cls:
                del_flag[i] = True
        ...
        if config.OUTPUT_SAM_CLS:
            data_dict['common_classes'] = config.COMMON_CLS
            data_dict['novel_classes'] = sampled_cls


        return data_dict
    
    def sample_Nways_with_prior(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.sample_Nways_with_prior, config=config)
        
        # points = data_dict['points']
        names = data_dict['gt_names']
        del_flag = np.zeros(names.shape[0], dtype=np.bool_)
        assert config.NOVEL_CLS_SELECTED is not None
        sampled_cls = config.NOVEL_CLS_SELECTED
        saved_cls = config.COMMON_CLS + sampled_cls
        for i in range(names.shape[0]):
            if names[i] not in saved_cls:
                del_flag[i] = True
