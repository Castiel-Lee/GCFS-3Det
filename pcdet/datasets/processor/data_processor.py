...
class DataProcessor(object):
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        ...
        if data_dict.get('points_org', None) is not None:
            points_org = data_dict['points_org']
            voxel_output_org = self.voxel_generator.generate(points_org)
            voxels_org, coordinates_org, num_points_org = voxel_output_org

            if not data_dict['use_lead_xyz']:
                voxels_org = voxels_org[..., 3:]  # remove xyz in voxels(N, 3)

            data_dict['voxels_org'] = voxels_org
            data_dict['voxel_coords_org'] = coordinates_org
            data_dict['voxel_num_points_org'] = num_points_org
        return data_dict
