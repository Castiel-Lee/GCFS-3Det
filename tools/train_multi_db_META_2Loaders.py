...
from train_utils.train_multi_db_META_2Loader import train_multi_db_model, train_multi_db_model_META
...
def parse_config():
    ...
    parser.add_argument('--batch_size', type=int, default=None, required=True, help='batch size for training')
    ...
    parser.add_argument('--batch_size_sup', type=int, default=None, required=True, help='batch size for support data')
    parser.add_argument('--batch_META_use', action='store_true', default=False, required=False, help='batch for META train')
    parser.add_argument('--FT_roi_only', action='store_true', default=False, required=False, help='only finetune roi_head module')
    parser.add_argument('--learning_rate', type=float, default=None, required=False, help='learning_rate') # 
...
def main():
    ...
    args.batch_size = args.batch_size*2 if args.batch_META_use else args.batch_size
    args.batch_size_sup = args.batch_size_sup*2 if args.batch_META_use else args.batch_size_sup
    ...
    # -------------------pass config of support dataset to model config---------------------------
    if cfg.get('DATA_CONFIG_SUPPORT', None):
        cfg.MODEL['DATA_CONFIG_SUPPORT'] = cfg.DATA_CONFIG_SUPPORT
    ...
    source_set_2, source_loader_2, source_sampler_2 = build_dataloader(
        ...
        batch_size=args.batch_size_sup,
        dist=dist_train, workers=args.workers,
        logger=logger,
        training=True,
        merge_all_iters_to_one_epoch=args.merge_all_iters_to_one_epoch,
    ...
    model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=source_set, class_names=cfg.CLASS_NAMES)
    ...
    if args.FT_roi_only:
        unfrozen_modules = ['roi_head']
        for name, param in model.named_parameters():
            for unfrozen_module in unfrozen_modules:
                if unfrozen_module in name:
                    param.requires_grad_(True)
                    break
                else:
                    param.requires_grad_(False)
        logger.info('Freezing model parameters except %s parameters.' % unfrozen_modules)
    ...
    train_func = train_multi_db_model_META if args.batch_META_use else train_multi_db_model
    # -----------------------start training---------------------------
